...
<!--[if gt IE 8]><!--> <html class="no-js" lang=""> <!--<![endif]-->
    ...
    <center><h1><span id="status" class="label {{$default_status_class}}">{{$default_status_text}}</span></h1></center>
            ...
            <div class="panel {{$page["class"]}}">
                ...
                <div class="panel-body">
                    <?php foreach ($page["components"] as $id => $object): ?>
                        <?php
                            if(array_key_exists('name', $object)) $name = $object["name"]; else $name = "";
	...
	<br />
        ...
        <script>
            function goto(name){var p = false;if(name=="")return;if(name.startsWith('#'))p=true;name=name.substring(1);var routeUrl="{{route('panel.route', ['id'=>$server_id,'route'=>null])}}"+name;var pageUrl="{{route('panel.page', ['id'=>$server_id,'page'=>null])}}"+name;if(p){window.location.assign(pageUrl);}else{window.location.assign(routeUrl);}}
        </script>
        <script>
            var $old_class = "{{$default_status_class}}";
            function setStatus(text, $class){var status = $("#status");status.text(text);status.removeClass($old_class);status.addClass($class);}
        </script>
        ...
        <script>
            $(document).ready(function(){
                $("#service-nickname").submit(function(event){
                    ...
                    $.ajax({
                        url: '{{route("service.nickname", ["id"=>$placeholders["server.id"]])}}',
                        type: 'POST',
                        dataType: 'json',
                        data: {
                            'nickname': $('#service-nickname>div>div>input[name=nickname]')[0].value
                        },
                        headers: {
                            'X-CSRF-TOKEN': $("#service-nickname>input#token")[0].value
                        }
                    });
                });
            });
